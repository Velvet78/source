...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #RubbleTEST2\r");
            Send(hSocket, "WHO #RubbleTEST2\r");
        } else {
            // randomly join #Rubble00-#Rubble05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #Rubble%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #Rubble%02d\r", channel_number).c_str());
        }
