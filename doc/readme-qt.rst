...
Alternatively, install Qt Creator and open the `Rubble-qt.pro` file.

An executable named `Rubble-qt` will be built.
...
A warning for people using the *static binary* version of Rubble on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of Rubble is linked against libdb5.3.
...
installed causes Rubble-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
